...
if __name__ == '__main__':
    ...
    date_start = datetime.datetime(2020, 8, 7,10)
    # --- Loop for each Critical Server
    Nbr = mycol.find({'Last Response Received':{'$gte': date_start}}).count()
    if Nbr == 0 :
       print ('No Records To Process')
       exit(0)
    Docs = pd.DataFrame(list(mycol.find({'Last Response Received':{'$gte': date_start}})))
...
(env)
sonargd@sdcpiapplnx143 NoTraffic
$
